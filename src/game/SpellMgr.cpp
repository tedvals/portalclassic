...
struct SpellRankHelper
{
	...
	{
		const SpellEntry* spell = GetSpellTemplate(spell_id);
		if (!spell)
		{
...
void SpellMgr::LoadSpellProcItemEnchant()
{
	...
	do
	{
		...
		SpellEntry const* spellInfo = GetSpellTemplate(entry);
...
void SpellMgr::LoadSpellBonuses()
{
	...
	do
	{
		...
		SpellEntry const* spell = GetSpellTemplate(entry);
		if (!spell)
		{
...
{
	SpellEntry const* spellInfo_2 = GetSpellTemplate(spellId_2);
...
SpellEntry const* SpellMgr::SelectAuraRankForLevel(SpellEntry const* spellInfo, uint32 level) const
{
	...
	{
		SpellEntry const* nextSpellInfo = GetSpellTemplate(nextSpellId);
		if (!nextSpellInfo)
			break;
...
void SpellMgr::LoadSpellChains()
{
	...
	// load known data from spell abilities
	{
		...
		for (SkillLineAbilityMap::const_iterator ab_itr = mSkillLineAbilityMap.begin(); ab_itr != mSkillLineAbilityMap.end(); ++ab_itr)
		{
			...
			SpellEntry const* spell_entry = GetSpellTemplate(spell_id);
			...
			// some forward spells not exist and can be ignored (some outdated data)
			SpellEntry const* forward_entry = GetSpellTemplate(forward_id);
			if (!forward_entry)
				continue;
	...
	do
	{
		...
		if (node.prev != 0 && !GetSpellTemplate(node.prev))
		{
		...
		if (!GetSpellTemplate(node.first))
		{
		...
		if (node.req != 0 && !GetSpellTemplate(node.req))
		{
...
void SpellMgr::LoadSpellLearnSkills()
{
	...
	{
		bar.step();
		SpellEntry const* entry = GetSpellTemplate(spell);
...
void SpellMgr::LoadSpellLearnSpells()
{
	...
	do
	{
		...
		if (!GetSpellTemplate(node.spell))
		{
	...
	for (uint32 spell = 0; spell < sSpellTemplate.GetMaxEntry(); ++spell)
	{
		...
		for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
		{
			if (entry->Effect[i] == SPELL_EFFECT_LEARN_SPELL)
			{
				...
				if (!GetSpellTemplate(dbc_node.spell))
					continue;
...
void SpellMgr::LoadSpellScriptTarget()
{
	...
	{
		SpellEntry const* spellProto = GetSpellTemplate(itr->spellId);
		if (!spellProto)
		{
...
void SpellMgr::LoadSpellPetAuras()
{
	...
	do
	{
		...
		else
		{
			...
			SpellEntry const* spellInfo2 = GetSpellTemplate(aura);
			if (!spellInfo2)
			{
...
bool SpellMgr::IsSpellValid(SpellEntry const* spellInfo, Player* pl, bool msg)
{
	...
	for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
	{
		...
		{
			SpellEntry const* spellInfo2 = GetSpellTemplate(spellInfo->EffectTriggerSpell[i]);
...
void SpellMgr::LoadSpellAreas()
{
	...
	do
	{
		...
		{
			SpellEntry const* spellInfo = GetSpellTemplate(abs(spellArea.auraSpell));
			if (!spellInfo)
			{
...
void SpellMgr::CheckUsedSpells(char const* table) const
{
	...
	do
	{
		...
		if (spell)
		{
			...
			SpellEntry const* spellEntry = GetSpellTemplate(spell);
			if (!spellEntry)
			{
		...
		else
		{
			...
			{
				SpellEntry const* spellEntry = GetSpellTemplate(spellId);
				if (!spellEntry)
					continue;
