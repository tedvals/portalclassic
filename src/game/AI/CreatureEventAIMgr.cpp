        ...
        do
        {
            ...
            switch (temp.event_type)
            {
                ...
                case EVENT_T_SPELLHIT:
                    ...
                    {
                        SpellEntry const* pSpell = GetSpellTemplate(temp.spell_hit.spellId);
                        if (!pSpell)
                        {
                ...
                case EVENT_T_FRIENDLY_MISSING_BUFF:
                {
                    SpellEntry const* pSpell = GetSpellTemplate(temp.friendly_buff.spellId);
                    if (!pSpell)
                    {
                ...
                {
                    SpellEntry const* pSpell = GetSpellTemplate(temp.buffed.spellId);
                    if (!pSpell)
                    {
            ...
            for (uint32 j = 0; j < MAX_ACTIONS; ++j)
            {
                ...
                switch (action.type)
                {
                    ...
                    case ACTION_T_CAST:
                    {
                        const SpellEntry* spell = GetSpellTemplate(action.cast.spellId);
                        if (!spell)
                    ...
                    case ACTION_T_CAST_EVENT:
                        ...
                        if (!GetSpellTemplate(action.cast_event.spellId))
