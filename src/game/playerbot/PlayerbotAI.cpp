...
bool PlayerbotAI::CanReachWithSpellAttack(Unit* target)
{
    ...
    for (SpellRanges::iterator itr = m_spellRangeMap.begin(); itr != m_spellRangeMap.end(); ++itr)
    {
        ...
        const SpellEntry* spellInfo = GetSpellTemplate(spellId);
        if (!spellInfo)
            continue;
...
uint32 PlayerbotAI::getSpellId(const char* args, bool master) const
{
    ...
    for (PlayerSpellMap::iterator itr = m_bot->GetSpellMap().begin(); itr != m_bot->GetSpellMap().end(); ++itr)
    {
        ...
        const SpellEntry* pSpellInfo = GetSpellTemplate(spellId);
        if (!pSpellInfo)
            continue;
...
uint32 PlayerbotAI::initSpell(uint32 spellId)
{
    ...
    for (SpellChainMapNext::const_iterator itr = nextMap.lower_bound(spellId); itr != nextMap.upper_bound(spellId); ++itr)
    {
        ...
        if (GetSpellTemplate(spellId)->SpellIconID != GetSpellTemplate(itr->second)->SpellIconID)
            continue;
    ...
    if (next == 0)
    {
        const SpellEntry* const pSpellInfo = GetSpellTemplate(spellId);
...
{
	if (SpellSpecific(spec) == SPELL_BLESSING || SpellSpecific(spec) == SPELL_AURA || SpellSpecific(spec) == SPELL_STING || SpellSpecific(spec) == SPELL_ASPECT || SpellSpecific(spec) == SPELL_CURSE || 
		SpellSpecific(spec) ==  SPELL_JUDGEMENT)
	{
