...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* pVictim, SpellEntry const* spell)
{
	...
	for (AuraList::const_iterator i = ignore.begin(); i != ignore.end(); ++i)
	{
		...
		{
		case MELEE_HIT_DODGE: canDodge = false; 
		case MELEE_HIT_BLOCK: canBlock = false;
		case MELEE_HIT_PARRY: canParry = false;
		default:
	...
	{
		if (canDodge && pVictim->CanDodgeAbility(this, spell))
		{
			tmp += uint32(pVictim->CalculateAbilityDodgeChance(this, spell) * 100);
			if (roll < tmp)
				return SPELL_MISS_DODGE;
		}
		if (canParry && pVictim->CanParryAbility(this, spell))
		{
		...
		if (canBlock && pVictim->CanBlockAbility(this, spell, true))
		{
