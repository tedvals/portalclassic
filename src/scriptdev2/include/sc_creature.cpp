...
SpellEntry const* ScriptedAI::SelectSpell(Unit* pTarget, int32 uiSchool, int32 iMechanic, SelectTarget selectTargets, uint32 uiPowerCostMin, uint32 uiPowerCostMax, float fRangeMin, float fRangeMax, SelectEffect selectEffects)
{
    ...
    for (uint8 i = 0; i < 4; ++i)
    {
		...
		if (!pTempSpell)
			pTempSpell = GetDBCSpellStore()->LookupEntry(m_creature->m_spells[i]);
...
{
	if (GetSpellStore()->GetMaxEntry() > 0)
	{
	...
	{
		SpellSummary = new TSpellSummary[GetDBCSpellStore()->GetNumRows()];

		SpellEntry const* pTempSpell;

		for (uint32 i = 0; i < GetDBCSpellStore()->GetNumRows(); ++i)
		{
			SpellSummary[i].Effects = 0;
			SpellSummary[i].Targets = 0;

			pTempSpell = GetDBCSpellStore()->LookupEntry(i);
			// This spell doesn't exist
			if (!pTempSpell)
				continue;
